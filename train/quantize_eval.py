...
`quant` block in model.json) — the same thing `spellman-train --store`
exports — so `assess` / `spellman` on the output dir exercises the actual
loader dequant path end to end:

    uv run python quantize_eval.py --store int8-row --out /tmp/model-int8
...
def main() -> None:
    ...
    ap.add_argument("--model", type=Path, default=Path(__file__).parent.parent / "model")
    ap.add_argument("--store", default="int8-row", help="f16 | int8-row | int8-col | fp8-row | fp8-col")
    ap.add_argument("--out", type=Path, default=Path(__file__).parent.parent / "model-int8")
    args = ap.parse_args()

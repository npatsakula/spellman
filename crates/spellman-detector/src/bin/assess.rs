...
fn parse_args() -> Result<Args, String> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--dump-per-lang" => {
                dump_per_lang = args.next().ok_or("--dump-per-lang requires a number")?.parse().map_err(|_| "bad --dump-per-lang")?;
            }
    ...
    Ok(Args {
        ...
        eval_files: if eval_files.is_empty() { Err("no eval files given")? } else { eval_files },
        dump_errors,
        dump_per_lang,
    })
}
...
impl Tally {
    ...
    fn accuracy(&self) -> f64 {
        if self.overall.1 == 0 { f64::NAN } else { self.overall.0 as f64 / self.overall.1 as f64 * 100.0 }
    }
...
fn word_tokens(text: &str) -> Vec<&str> {
    text.split_whitespace().filter(|t| t.chars().any(char::is_alphabetic)).collect()
}
...
fn tally_fragments(detector: &mut BulkDetector, items: &[(Lang, String)]) -> Result<Tally, Box<dyn std::error::Error>> {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (&gold, &count) in &gold_total {
        text_tally.per_lang.insert(gold, (correct.get(&gold).copied().unwrap_or(0), count));
    }
    ...
    let accs: Vec<f64> =
        length_buckets.iter().map(|(c, t)| if *t == 0 { f64::NAN } else { *c as f64 / *t as f64 * 100.0 }).collect();
    println!(
    ...
    langs.sort_by(|a, b| word_tally.lang_accuracy(*a).total_cmp(&word_tally.lang_accuracy(*b)));
    ...
    if langs.len() > 15 {
        println!("  (+{} languages below the 15 shown, all ≥ {:.2}% on words)", langs.len() - 15, word_tally.lang_accuracy(langs[15]));
    }
    ...
    f1_langs.sort_by_key(|l| f1(*l, &correct, &gold_total, &pred_total).total_cmp(&0.0).reverse());
    for lang in f1_langs.iter().filter(|l| f1(**l, &correct, &gold_total, &pred_total) < 0.999) {
        let p = precision(*lang, &correct, &pred_total);
        let r = recall(*lang, &correct, &gold_total);
        println!("  {:>4}  {:>5}  {:.2}  {:.2}  {:.2}", lang, gold_total[lang], p, r, f1(*lang, &correct, &gold_total, &pred_total));
    }
    let perfect = f1_langs.iter().filter(|l| f1(**l, &correct, &gold_total, &pred_total) >= 0.999).count();
    println!("  (+{perfect} languages with F1 ≥ 99.9%)");
    println!();
    println!("top confusions (gold -> predicted):");
    let mut pairs: Vec<((Lang, Lang), usize)> =
        confusion.counts.iter().filter(|((g, p), _)| g != p).map(|(k, v)| (*k, *v)).collect();
